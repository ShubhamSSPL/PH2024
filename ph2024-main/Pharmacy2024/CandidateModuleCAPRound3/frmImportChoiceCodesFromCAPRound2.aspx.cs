...
{
    public partial class frmImportChoiceCodesFromCAPRound2 : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                try
                {
                    ...
                    DataSet dsCurrentLink = reg.getCurrentLinkCAPRound3(objSessionData.PID, "frmImportChoiceCodesFromCAPRound2.aspx");
        ...
        protected void ImportOptions_CheckedChanged(object sender, EventArgs e)
        {
            ShowMessage shInfo = (ShowMessage)Master.FindControl("ShowMsg");
            try
            {
                ...
                if (rbnYes.Checked)
                {
                    trOptionsList1.Visible = true;
                    trOptionsList2.Visible = true;

                    gvOptionsList.DataSource = reg.getImportOptionsFromCAPRound2(objSessionData.PID);
                    gvOptionsList.DataBind();
                }
                else
                {
                    trOptionsList1.Visible = false;
                    trOptionsList2.Visible = false;
                }
            }
            catch (Exception ex)
            {
        ...
        protected void btnProceed_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (rbnYes.Checked)
                {
                    if (reg.importOptionsFromCAPRound2(objSessionData.PID, Session["UserLoginID"].ToString(), UserInfo.GetIPAddress()))
                    {
                        Response.Redirect("../CandidateModuleCAPRound3/frmShortListOptions.aspx");
                    }
                    else
                    {
                        Response.Redirect("../CandidateModuleCAPRound3/frmShortListOptions.aspx");
