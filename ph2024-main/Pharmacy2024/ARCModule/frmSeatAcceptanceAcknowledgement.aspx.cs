...
{
    public partial class frmSeatAcceptanceAcknowledgement : System.Web.UI.Page
    {
            ...
            try
            {
                if (!IsPostBack)
                {
                    ...
                    if (dsSeatAcceptanceStatusDetails.Tables[0].Rows.Count > 0)
                    {
                        ...
                        if (dsSeatAcceptanceStatusDetails.Tables[0].Rows[0]["ReportingStatus"].ToString() != "Confirmed")
                        {
                            Response.Redirect("../ApplicationPages/WelcomePageARC.aspx", true);
                        }
