...
namespace Pharmacy2024.CandidateModuleCAPRound1
{
    public partial class frmOptionForm : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                if (!IsPostBack)
                {
                    ...
                    DataSet ds = reg.getCurrentLinkCAPRound1(objSessionData.PID, "frmOptionForm");
                    ...
                    {
                        DataSet dsPersonalInformation = reg.getPersonalInformationCAPRound1(objSessionData.PID);

                        lblApplicationID.Text += dsPersonalInformation.Tables[0].Rows[0]["ApplicationID"].ToString();
                        lblVersionNo.Text += dsPersonalInformation.Tables[0].Rows[0]["VersionNoCAPRound1"].ToString();
                        ...
                        lblFirstChoice.Text = getFirstChoice(objSessionData.PID);
                        DataSet dsPreferancedOptionsList = reg.getPreferancedOptionsListForDisplayCAPRound1(objSessionData.PID);
                        ...
                        DataSet dsInstructions = reg.getInstructionsCAPRound1(objSessionData.PID);
                        ...
                        DateTime LastModifiedOn = Convert.ToDateTime(dsPersonalInformation.Tables[0].Rows[0]["LastModifiedOnCAPRound1"].ToString());
                        lblLastModifiedOn.Text = LastModifiedOn.ToString("dd/MM/yyyy") + "  " + String.Format("{0:T}", LastModifiedOn);
                        lblLastModifiedBy.Text = dsPersonalInformation.Tables[0].Rows[0]["LastModifiedByCAPRound1"].ToString() + ", " + dsPersonalInformation.Tables[0].Rows[0]["LastModifiedByIPAddressCAPRound1"].ToString();

                        DataSet dsOptionFormConfirmationDetails = reg.getOptionFormConfirmationDetailsCAPRound1(objSessionData.PID);
        ...
        private string getFirstChoice(long PID)
        {
            string firstChoice = "";
            DBConnection db = new DBConnection();
            DataSet ds = db.ExecuteDataSet("sp_getFirstChoiceGivenCapRound1", new SqlParameter[]
            {
            ...
            {
                firstChoice = ds.Tables[0].Rows[0][0].ToString();
            }
            return firstChoice;
        }
    }
