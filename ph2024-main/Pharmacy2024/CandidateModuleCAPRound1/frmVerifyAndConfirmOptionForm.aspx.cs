    ...
    public partial class frmVerifyAndConfirmOptionForm : System.Web.UI.Page
    {
            ...
            if (!IsPostBack)
            {
                try
                {
                    ...
                    if (PreferenceCount > 300)
                    {
                        ...
                        Response.Redirect("../CandidateModuleCAPRound1/frmShortListOptions?tms=101");
                        shInfo.SetMessage("You Can Not Add More Than 300 Options. You Can Add Only  " + "(" + (300) + ")" + "  Option(s) Now.", ShowMessageType.Information);
                    }

                    string strInvalidChoiceCodes = reg.getInvalidChoiceCodeListCAPRound1(objSessionData.PID);
                    ...
                    //DataSet ds = reg.getPreferancedOptionsListCAPRound1(objSessionData.PID);
        ...
        protected void btnVerifyOtp_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    if (reg.verifyOTP(ApplicationID, txtOneTimePassword.Text, 9))
                    {
                        if (reg.isCandidateSelectedOptionsCAPRound1(objSessionData.PID))
                        {
                            switch (reg.confirmOptionFormCAPRound1(objSessionData.PID, DataEncryption.EncryptDataByHashSHA1(hdnPassword.Value), Session["UserLoginID"].ToString(), UserInfo.GetIPAddress()))
                            {
                                case 0:
                                ...
                                case 2:
                                    ...
                                    //SMS objSMS = new SMS();
                                    //DataSet ds = reg.GetSMSEmailContent(reg.getPersonalID(ApplicationID), "OptionFormConfirmCAPI", UserInfo.GetIPAddress(), Global.IsOTPVerificationRequired);
                                    //if (ds.Tables[1] != null)
                                    //{
                                    ...
                                    break;
                                case 3:
                                    if (objSessionData.StepIDCAPRound1 < 4)
                                    {
                                        objSessionData.StepIDCAPRound1 = 4;
                                    }

                                    if (objSessionData.OptionFormStatusCAPRound1 == 'C')
                                    {
                                        objSessionData.OptionFormStatusCAPRound1 = 'A';
                                    }
                                    ...
                                    //DataSet ds1 = reg.GetSMSEmailContent(objSessionData.PID, "OptionFormConfirmCAPI", UserInfo.GetIPAddress(), Global.IsOTPVerificationRequired);
            ...
            try
            {
                ...
                if (result > 0)
                {
                    if (Global.IsOTPVerificationRequiredForCancelApplicationForm == 1)
                    {
                        ...
                        sMSTemplate.PID = PID;
                        SynCommon synCommon = new SynCommon();
                        if (synCommon.SendOTPSMS(sMSTemplate, SynCommon.TemplateSystemType.CAPOptionFormIOTP))
                        {
                        ...
                        //SMS objSMS = new SMS();
                        //DataSet ds = reg.GetSMSEmailContent(objSessionData.PID, "CAPOptionFormI", UserInfo.GetIPAddress(), Global.IsOTPVerificationRequired);
                        //if (ds.Tables[1] != null)
                        //{
                        //    if (ds.Tables[1].Rows.Count > 0)
                        //    {
                    ...
                    {
                        switch (reg.confirmOptionFormCAPRound1(objSessionData.PID, DataEncryption.EncryptDataByHashSHA1(txtPassword.Text), Session["UserLoginID"].ToString(), UserInfo.GetIPAddress()))
                        {
                            ...
                            case 2:
                                ...
                                sMSTemplate.CurrentDateTime = DateTime.Now.ToString();
                                if (Global.SendSMSToCandidate == 1)
                                    synCommon.SendInformationSMS(sMSTemplate, SynCommon.TemplateSystemType.OptionFormConfirmCAPI);
                                //SMS objSMS = new SMS();
                                //DataSet ds = reg.GetSMSEmailContent(objSessionData.PID, "OptionFormConfirmCAPI", UserInfo.GetIPAddress(), Global.IsOTPVerificationRequired);
                                //if (ds.Tables[1] != null)
                                //{
                                //    if (ds.Tables[1].Rows.Count > 0)
                                //    {
                                ...
                                //}
                                //SMS objSMS3 = new SMS();
                                //string MobileNo3 = DataEncryption.DecryptDataByEncryptionKey(reg.getMobileNo(objSessionData.PID));
                                //string Message3 = "FE9" + objSessionData.PID.ToString() + ", Your Option Form for CAPRound 1 is Confirmed  by " + Session["UserLoginID"].ToString() + " on " + DateTime.Now.ToShortDateString() + " " + DateTime.Now.ToShortTimeString() + " for Post HSC Diploma Admissions. DTE, MH";
                                //objSMS3.sendSingleSMS(MobileNo3, Message3);
                            ...
                            case 3:
                                ...
                                sMSTemplate1.CurrentDateTime = DateTime.Now.ToString();
                                if (Global.SendSMSToCandidate == 1)
                                    synCommon1.SendInformationSMS(sMSTemplate1, SynCommon.TemplateSystemType.OptionFormConfirmCAPI);
                                //SMS objSMS1 = new SMS();
                                //string MobileNo = DataEncryption.DecryptDataByEncryptionKey(reg.getMobileNo(objSessionData.PID));
                                //string Message = "FE19" + objSessionData.PID.ToString() + ", Your Option Form for CAPRound 1 is Confirmed  by " + Session["UserLoginID"].ToString() + " on " + DateTime.Now.ToShortDateString() + " " + DateTime.Now.ToShortTimeString() + " for Post HSC Diploma Admissions. DTE, MH";
                                //objSMS.sendSingleSMS(MobileNo, Message);
