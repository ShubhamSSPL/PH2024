    ...
    public partial class frmSetPreferences : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                try
                {
                    ...
                    DataSet dsCurrentLink = reg.getCurrentLinkCAPRound1(objSessionData.PID, "frmSetPreferences.aspx");
                    ...
                    if (objSessionData.OptionFormStatusCAPRound1 == 'A' || objSessionData.StepIDCAPRound1 < 1)
                    {
                        Response.Redirect("../CandidateModuleCAPRound1/frmOptionForm.aspx?tms=101", true);
                    }

                    if (objSessionData.StepIDCAPRound1 >= 2)
                    {
                    ...
                    dgSetPreferences.DataSource = reg.getSelectedOptionsListCAPRound1(objSessionData.PID);
                    CheckCount = 0;
                    dgSetPreferences.DataBind();
                    if (dgSetPreferences.Items.Count == 0)
                    {
                        Response.Redirect("../CandidateModuleCAPRound1/frmShortListOptions.aspx?tms=101", true);
                    }
                }
                catch (Exception ex)
                {
        ...
        protected void btnProceed_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                switch (reg.savePreferencesCAPRound1(objSessionData.PID, PreferencesList, Session["UserLoginID"].ToString(), UserInfo.GetIPAddress()))
                {
                    case 0:
                        shInfo.SetMessage("Error In Saving Sequences. Please try again!!!", ShowMessageType.UserError);
                        break;
                    case 1:
                        if (objSessionData.StepIDCAPRound1 < 2)
                        {
                            objSessionData.StepIDCAPRound1 = 2;
                        }
                        ...
                        break;
                    case 2:
                        Session["StepIDCAPRound1"] = 3;
                        Session["OptionFormStatusCAPRound1"] = "A";
