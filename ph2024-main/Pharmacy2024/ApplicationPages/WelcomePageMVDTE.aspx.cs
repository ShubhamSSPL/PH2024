...
{
    public partial class WelcomePageMVDTE : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                try
                {
                    ...
                    {
                        lblInst.Text = dsSOStats.Tables[0].Rows[0]["NoOfinstAssigned"].ToString();
                        lblMVSubmitted.Text = dsSOStats.Tables[0].Rows[0]["NoOfCandidates"].ToString();
                        lblROVerified.Text = dsSOStats.Tables[0].Rows[0]["CandidatesVerified"].ToString();
                        lblDTEVerified.Text = dsSOStats.Tables[0].Rows[0]["CandidatesPending"].ToString();
                    }
                }
                catch (Exception ex)
                {
