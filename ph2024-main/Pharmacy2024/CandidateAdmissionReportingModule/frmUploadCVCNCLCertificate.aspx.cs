...
namespace Pharmacy2024.CandidateAdmissionReportingModule
{
    public partial class frmUploadCVCNCLCertificate : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            AjaxPro.Utility.RegisterTypeForAjax(typeof(frmUploadCVCNCLCertificate));
            if (!IsPostBack)
            {
                       ...
                       )
                {
                    ...
                    if (dsCandidateDetailsForDisplay.Tables[0].Rows.Count > 0)
                    {
                        ...
                        else
                        {
                            ...
                            shInfo.SetMessage("The Link has been closed to Upload Original document of CVC/TVC/NCL/EWS against the Receipt of CVC/TVC/NCL/EWS(except SEBC and OBC Candidates).", ShowMessageType.Information);
                        }
                ...
                if (verifycnt > 0)
                {
                    if (gvCVCNCL.Rows.Count == 2 && (verifycnt == 2 || (lblCategoryForAdmission.Text.ToString() == "SEBC$#" || lblCategoryForAdmission.Text.ToString() == "OBC$#"))) //NCL CVC
                    {
                        if (UploadCVCNCLCertificate(CVCStatus, NCLStatus, EWSStatus))
                        {
                            ...
                            ContentBox1.Visible = false;
                            ContentBox2.Visible = false;
                            shInfo.SetMessage("Your CVC/TVC, NCL  Certificate has been Uploaded and eSC will verify Soon. You are able to do your Self Verification and Seat Acceptance after Scrutiny done by the eSC officer.", ShowMessageType.Information);
                        }
                    }
                    else if (gvCVCNCL.Rows.Count == 1 && verifycnt == 1)
                    {
                        if (UploadCVCNCLCertificate(CVCStatus, NCLStatus, EWSStatus))
                        {
                            ...
                            ContentBox1.Visible = false;
                            ContentBox2.Visible = false;
                            shInfo.SetMessage("Your CVC/TVC, NCL or EWS Certificate has been Uploaded and eSC will verify Soon. You are able to do your Self Verification and Seat Acceptance after Scrutiny done by the eSC officer.", ShowMessageType.Information);
                        }
                    }
