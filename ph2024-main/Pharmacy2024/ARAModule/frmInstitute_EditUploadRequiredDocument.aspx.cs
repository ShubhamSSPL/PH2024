...
{
    public partial class frmInstitute_EditUploadRequiredDocument : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            AjaxPro.Utility.RegisterTypeForAjax(typeof(frmInstitute_EditUploadRequiredDocument));
            if (!IsPostBack)
            {
                SessionData objSessionData = (SessionData)Session["SessionData"];
                try
                {
                    hdnCategoryID.Value = reg.getCategoryID(PID).ToString();
        ...
        protected void gvDocuments_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            if (e.CommandName == "D")
            {
                ...
                {
                    SessionData objSessionData = (SessionData)Session["SessionData"];
                    ds = new dbUtility().deleteUploadedDocument(Convert.ToInt64(PID), DocId, Session["UserLoginID"].ToString(), UserInfo.GetIPAddress());

                    if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0 && ds.Tables[0].Rows[0]["Status"].ToString() == "1")
                    {
                        LoadDocuments(Convert.ToInt64(PID));
                    }
                    else
                    {
            ...
            if (flUpload.HasFile)
            {
                ...
                if (fileurl != "")
                {
                    ...
                    try
                    {
                        ...
                        int JuridictionID = 0;
                        int TalukaID = 0;
                        ds = new dbUtility().saveUploadedDocument(objSessionData.PID, DocID, sysfilename, uploadedFileURL, originalFileName, Convert.ToString(Session["UserLoginID"]), UserInfo.GetIPAddress(), checkBarcode, barcode);
                        if (ds != null && ds.Tables.Count > 0)
                        {
        ...
        [AjaxMethod]
        public DataSet FillJuridiction(string documentID, string PID)
        {
            ...
            try
            {
                ...
                if (documentID == "22")
                {
                    if (reg.getCategoryID(Convert.ToInt64(PID)) == 3)
                    {
        ...
        [AjaxMethod]
        public string getCategoryID(string PID)
        {
            SessionData objSessionData = (SessionData)Session["SessionData"];
            return reg.getCategoryID(Convert.ToInt64(PID)).ToString();
        }

        [AjaxMethod]
        public string UploadDoc(string base64String, string PID, string documentID, string hidJUD, string hidDID, string hidTID, string fileName, string fileExt, char checkBarcode, string barcode)
        {    
            ...
            if (base64String.Length > 7)
            {
                ...
                if (extension != string.Empty)
                {
                    ...
                    string sysfilename = PID + "_" + DateTime.Now.Ticks.ToString() + extension;
                    ...
                    if (fileurl != "")
                    {
                        ...
                        try
                        {
                            ...
                            int JuridictionID = 0;
                            int TalukaID = 0;
                            ds = new dbUtility().saveUploadedDocument(Convert.ToInt64(PID), Convert.ToInt16(documentID), sysfilename, uploadedFileURL, originalFileName, Convert.ToString(Session["UserLoginID"]), UserInfo.GetIPAddress(), checkBarcode, barcode);
                            if (ds != null && ds.Tables.Count > 0)
                            {
                                if (ds.Tables[0].Rows.Count > 0 && ds.Tables[0].Rows[0]["Status"].ToString() == "1")
                                {
                                    if (Convert.ToInt16(documentID) == 21 || Convert.ToInt16(documentID) == 22)
                                    {
                                        ...
                                        dsJ = new dbUtility().saveUploadedDocumentJuridiction(Convert.ToInt64(PID), Convert.ToInt16(documentID), Convert.ToString(Session["UserLoginID"]), UserInfo.GetIPAddress(), JuridictionID, DistrictID, TalukaID);
                                        if (dsJ != null && dsJ.Tables.Count > 0)
                                        {
                                            if (dsJ.Tables[0].Rows.Count > 0 && dsJ.Tables[0].Rows[0]["Status"].ToString() == "1")
                                            {
        ...
        protected void btnProceed_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        Response.Redirect("../ARAModule/frmInstitute_VerificationCandidateDocuments?PID=" + PID + "&Code=" + PID.ToString().GetHashCode().ToString() +"&ChoiceCode=" + ChoiceCode + "&ChoiceCodeDisplay=" + ChoiceCodeDisplay, true);
                    }
                }
            }
            catch (Exception ex)
        ...
        private class dbUtility
        {
            public DataSet GetAllDocuments(Int64 PID)
            {
                ...
                try
                {
                    return db.ExecuteDataSet("MHDTE_spGetAllDocumentsForARA", param);
            ...
            public DataSet saveUploadedDocument(Int64 PID, Int64 DocumentID, string FileName, string AbsoluteFilePath, string OriginalFileName, string FileUploadedBy, string FileUploadedByIPAddress, char checkbarcode, string barcode)
            {
                SqlParameter[] param =
                    {
                    new SqlParameter("@PID",SqlDbType.BigInt),
                ...
                try
                {
                    return db.ExecuteDataSet("MHDTE_spUpdateDocumentUploadStatusForARA", param);
