...
{
    public partial class PendingGrievanceList : System.Web.UI.Page
    {
        ...
        protected void GetGrievanceList()
        {
            ...
            try
            {
                ...
                gvGrievanceList.DataSource = new BusinessServiceImp().GetGrievanceListByStatus(Convert.ToInt32(Session["UserTypeID"]), Session["UserLoginID"].ToString(), "P", txtSearch.Text, GrievanceCategoryID);
        ...
        protected void gvGrievanceList_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            try
            {
                ...
                if (GrievanceID > 0)
                {
                    string returnValue = new BusinessServiceImp().OpenGrievance(GrievanceID, Convert.ToInt32(Session["UserTypeID"]), Session["UserLoginID"].ToString(), UserInfo.GetIPAddress());
                    if (returnValue == "Y")
                    {
                        Response.Redirect("../Grievance/ViewGrievance.aspx?GID=" + GrievanceID + "&Flag=P", true);
                    }
                    else
                    {
                        GetGrievanceList();
