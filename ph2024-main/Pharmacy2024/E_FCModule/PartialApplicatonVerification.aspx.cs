...
{
    public partial class PartialApplicatonVerification : System.Web.UI.Page
    {
        ...
        {
            gvReport.DataSource = reg.GetApplicationFormListForPartialFCVerification(Session["UserLoginID"].ToString(), Convert.ToInt16(Session["UserTypeID"].ToString()));
            gvReport.DataBind();
            for (Int32 i = 0; i < gvReport.Rows.Count; i++)
            {
                //gvReport.Rows[i].Cells[0].Text = (i + 1).ToString() + ".";
                gvReport.Rows[i].Cells[4].Text = DataEncryption.DecryptDataByEncryptionKey(gvReport.Rows[i].Cells[4].Text);
            }
