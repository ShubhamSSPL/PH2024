...
{
    public partial class EVerificationNEETDetails : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            AjaxPro.Utility.RegisterTypeForAjax(typeof(EVerificationNEETDetails));
            AjaxPro.Utility.RegisterTypeForAjax(typeof(Pharmacy2024.ViewMyDocument));
            if (!IsPostBack)
            {
                try
                {
                    //JEEDetails obj = reg.getJEEDetails(objSessionData.PID);
                    ...
                    DataSet dsCETResult = reg.getCETDetails(obj.CETApplicationFormNo);
                    if (dsCETResult.Tables[0].Rows.Count > 0)
                    {
                        if (dsCETResult.Tables[0].Rows[0]["IsCandidateGivenCET"].ToString() == "Y")
                        {
                            ...
                            //lblCETCandidateName.Text = dsCETResult.Tables[0].Rows[0]["CandidateName"].ToString() + " " + "<font color = 'red' size='1'>(Name As Per MHTCET Result)</font>";
