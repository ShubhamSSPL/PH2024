...
{
    public partial class frmSeatAcceptanceStatus : System.Web.UI.Page
    {
        ...
        protected void btnSecretKey_Click(object sender, EventArgs e)
        {
            if (txtkey.Text == Application["SecretKey"].ToString())
            {
                ...
                Response.Redirect("frmSeatAcceptanceStatus");
            }
            else
            {
        ...
        public class dbUtilityOptionForm
        {
            public DataSet GetStatus()
            {
                ...
                try
                {
                    return db.ExecuteDataSet("sp_GetSelfSeatAcceptanceInsituteCnt", new SqlParameter[] { });
                }
                finally
                {
                    db.Dispose();
