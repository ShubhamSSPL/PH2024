...
{
    public partial class frmSeatAcceptanceStatusCapRound2 : System.Web.UI.Page
    {
        ...
        protected void btnSecretKey_Click(object sender, EventArgs e)
        {
            if (txtkey.Text == Application["SecretKey"].ToString())
            {
                ...
                Response.Redirect("frmSeatAcceptanceStatusCapRound2");
            }
            else
            {
        ...
        public class dbUtilityOptionForm
        {
            public DataSet GetStatus()
            {
                ...
                try
                {
                    return db.ExecuteDataSet("sp_GetSelfSeatAcceptanceInsituteCntCApRound2", new SqlParameter[] { });
                }
                finally
                {
                    db.Dispose();
