...
{
    public partial class frmCheckProvisionalMeritStatus : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                if (Flag != null && Flag == "CandidateLogin")
                {
                    ...
                    DataSet ds = reg.getProvisionalMeritStatus(Global.ApplicationFormPrefix + objSessionData.PID, DOB);
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        else
                        {
                            ...
                            if (Convert.ToInt64(ds.Tables[0].Rows[0]["StatePHMeritNo"].ToString()) > 0)
                            {
                                ...
                                lblStatePHMeritNo.Text = ds.Tables[0].Rows[0]["PHTypeForMerit"].ToString() + " - " + ds.Tables[0].Rows[0]["StatePHMeritNo"].ToString();
                            }
                            if (Convert.ToInt64(ds.Tables[0].Rows[0]["UniversityPHMeritNo"].ToString()) > 0)
                            {
                                trUniversityPHMeritNo.Visible = true;
                                lblUniversityPHMeritNo.Text = ds.Tables[0].Rows[0]["FinalHomeUniversity"].ToString() + " - " + ds.Tables[0].Rows[0]["PHTypeForMerit"].ToString() + " - " + ds.Tables[0].Rows[0]["UniversityPHMeritNo"].ToString();
                            }
                            ...
                            if (Convert.ToInt64(ds.Tables[0].Rows[0]["TFWSMeritNo"].ToString()) > 0)
                            {
                                ...
                                lblTFWSMeritNo.Text = ds.Tables[0].Rows[0]["TFWSMeritNo"].ToString();
                            }
                ...
                else
                {
                    ...
                    if (Global.DisplayProvisionalMeritList == 1)
                    {
                    ...
                    else
                    {
                        cbCheckResult.Visible = false;
                        cbDisplayResult.Visible = false;
                        shInfo.SetMessage("Provisional Merit List is Not Published.", ShowMessageType.Information);
                    }
                }
            }
        }
        protected void btnProceed_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                DBUtilitNEET regDB = new DBUtilitNEET();
                ...
                //else if (regDB.NEETScoreWongCandidatesBlocked(PID))
                //{
                //    cbCheckResult.Visible = true;
                //    cbDisplayResult.Visible = false;

                //    shInfo.SetMessage("You have Entered wrong NEET Percentile. To appear in final merit, kindly correct your entered NEET exam Percentile by raising the grievance (29/11/2021 to 01/12/2021 upto 05:00PM) through your login and get corrected NEET Percentile and all other details online or offline as per the scrutiny mode selected previously during application form filling.", ShowMessageType.Information);
                //}
                else
                {
                    cbCheckResult.Visible = true;
    ...
    public class DBUtilitNEET
    {
        public bool NEETScoreWongCandidatesBlocked(Int64 PID)
        {
            DBConnection db = new DBConnection();
            try
            {

                Int32 result = Convert.ToInt32(db.ExecuteScaler("MHDTE_spCheckNEETScoreWongCandidatesBlocked", new SqlParameter[]
                 {
