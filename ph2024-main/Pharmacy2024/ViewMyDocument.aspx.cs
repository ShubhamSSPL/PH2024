...
{
    public partial class ViewMyDocument : System.Web.UI.Page
    {
        ...
        private void ShowDocument()
        {
            ...
            DataSet ds = db.ExecuteDataSet("sp_GetDocumentDetails", new SqlParameter[] { new SqlParameter("@DocumentID", DocID) });
            db.Dispose();
            if (ds.Tables[0].Rows.Count > 0)
            {
                string dURL = ds.Tables[0].Rows[0]["AbsoluteFilePath"].ToString().ToLower();
                string dName = ds.Tables[0].Rows[0]["FileName"].ToString();
        ...
        [AjaxMethod]
        public string GetCandidateDocumentAsBase64(string blobFilePath)
        {
            AzureDocumentUpload objDU = new AzureDocumentUpload();
            string base64 = objDU.GetBlobContentAsBase64("studentdocuments", blobFilePath);
            return base64;
        }
    }
}