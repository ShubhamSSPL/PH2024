...
{
    public partial class frmInstVerifyARA : System.Web.UI.Page
    {
        ...
        public void fillGrid()
        {
            ...
            gvExcel.DataSource = reg.ARAReportforMV("F");
            gvExcel.DataBind();
            gvReport.DataSource = reg.ARAReportforMV("F");
            gvReport.DataBind();
                    ...
                    if (cnt != 0)
                    {
                        for (i = 0; i < gvReport.Rows.Count; i++)
                        {
                            ...
                            if (chkVerify.Checked == true)
                            {
                                ...
                                DataSet dsNotRecommendedForApprovalCandidate = reg.getNotRecommendedForApprovalCandidate(InstCode);
                                if (dsNotRecommendedForApprovalCandidate.Tables[0].Rows.Count > 0)
                                {
                                    for (int j = 0; j < dsNotRecommendedForApprovalCandidate.Tables[0].Rows.Count; j++)
                                    {
                                        SMSTemplate sMSTemplate = new SMSTemplate();
                                        SynCommon synCommon = new SynCommon();
                                        sMSTemplate.PID = Convert.ToInt64(dsNotRecommendedForApprovalCandidate.Tables[0].Rows[0]["PersonalID"].ToString());
