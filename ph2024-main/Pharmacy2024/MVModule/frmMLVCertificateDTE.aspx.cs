...
{
    public partial class frmMLVCertificateDTE : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (Request.QueryString["InstituteCode"] != null)
                {
                    ...
                    string Remark = reg.getCertificateRemarks(Request.QueryString["InstituteCode"].ToString(),'R');
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                Int64 InstCode = Convert.ToInt32(Request.QueryString["InstituteCode"].ToString()); ;
                string CreatedBy = Session["UserLoginID"].ToString();
                string CreatedIPAddress = UserInfo.GetIPAddress();
                string remarks = txtRemarks.Text.ToString();
                reg.saveInstituteVarifyRODTE(InstCode, 'Y', CreatedBy, CreatedIPAddress, 'D', remarks);
                ClientScript.RegisterClientScriptBlock(this.GetType(), "alert", "alert('Institute Verified.');", true);
