        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                try
                {
                    ...
                    gvList.DataSource = reg.getAFCList(UserTypeID, UserLoginID);
                    gvList.DataBind();
                }
                catch (Exception ex)
                {
        ...
        private string saveFile(string FilePath, Int32 FileNo)
        {
            ...
            else if (FileNo == 2)
            {
                if (fileInput2.PostedFile != null)
                {
                    ...
                    FileInfo objFile = new FileInfo(fileInput1.PostedFile.FileName);
                    string sysfilename = FilePath;//+ "_" + DateTime.Now.Ticks.ToString()
                    HttpFilePath = azureDocumentUpload.UploadDocument(ConfigurationManager.AppSettings["Project_Name"].ToLower(), sysfilename, objFileStream, fileInput2.PostedFile.ContentType,"Attachment".ToLower()); ;
                }
            }
