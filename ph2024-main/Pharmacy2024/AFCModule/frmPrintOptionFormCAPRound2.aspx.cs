...
{
    public partial class frmPrintOptionFormCAPRound2 : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                try
                {
                    ...
                    DataSet dsPersonalInformation = reg.getPersonalInformationCAPRound2(PID);
                    Int32 UserTypeID = Convert.ToInt32(PID);
                    DataSet version = new dbUtility().GetUnlockOptionFormversion(UserTypeID);
                    if (version.Tables[0].Rows.Count > 0 && version.Tables[0].Rows[0]["Previousversion"].ToString() != "")
                    {
                        lblApplicationID.Text += (dsPersonalInformation.Tables[0].Rows[0]["ApplicationID"].ToString() + " :- " + version.Tables[0].Rows[0]["Previousversion"].ToString());
                    }
                    else
                    {
                    ...
                    DataSet dsPreferancedOptionsList = reg.getPreferancedOptionsListForDisplayCAPRound2(PID);
                    ...
                    DataSet dsInstructions = reg.getInstructionsCAPRound2(PID);
                    ...
                    DataSet dsOptionFormConfirmationDetails = reg.getOptionFormConfirmationDetailsCAPRound2(PID);
        ...
        {
            public DataSet GetUnlockOptionFormversion(int UserTypeID)
            {
                SqlParameter[] param =
                    {
                ...
                try
                {
                    return db.ExecuteDataSet("MHDTE_spUnlockOptionFormCAPRound2_version", param);
