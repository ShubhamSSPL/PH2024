...
{
    public partial class frmUploadDocumentafterReporting : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            AjaxPro.Utility.RegisterTypeForAjax(typeof(frmUploadDocumentafterReporting));
            if (!IsPostBack)
            {
                ...
                if (ds != null && ds.Tables.Count > 0)
                {
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        if (Convert.ToString(ds.Tables[0].Rows[0]["Status"]) == "1")
                        {
                            ...
                            //else
                            //{
                            string candidateScrutinyType = reg.CheckCandidateFCVerificationFor(PID);
                            if (candidateScrutinyType == "E")
                            {
                                //gvDocuments.Columns[3].Visible = false;
                                //gvDocuments.Columns[5].Visible = false;
                            }
            ...
            if (flUpload.HasFile)
            {
                ...
                if (fileurl != "")
                {
                    ...
                    try
                    {
                        ...
                        int JuridictionID = 0;
                        int TalukaID = 0;
                        ds = new dbUtility().saveUploadedDocument(PID, DocID, sysfilename, uploadedFileURL, originalFileName, Convert.ToString(Session["UserLoginID"]), UserInfo.GetIPAddress(), checkBarcode, barcode);
                        if (ds != null && ds.Tables.Count > 0)
                        {
        ...
        [AjaxMethod]
        public string UploadDoc(string base64String, string documentID, string hidJUD, string hidDID, string hidTID, string fileName, string fileExt, char checkBarcode, string barcode, string SPID)
        {
            //ShowMessage shInfo = (ShowMessage)Master.FindControl("ShowMsg");
                ...
                if (extension != string.Empty)
                {
                    ...
                    if (fileurl != "")
                    {
                        ...
                        try
                        {
                            ...
                            int JuridictionID = 0;
                            int TalukaID = 0;
                            ds = new dbUtility().saveUploadedDocument(Convert.ToInt64(SPID), Convert.ToInt16(documentID), sysfilename, uploadedFileURL, originalFileName, Convert.ToString(Session["UserLoginID"]), UserInfo.GetIPAddress(), checkBarcode, barcode);
                            dsFlag = new dbUtility().UpdateDocumentIsSubmittedAtAFCFlagafterReporting(Convert.ToInt64(SPID), Convert.ToInt16(documentID));
                            if ((ds != null && ds.Tables.Count > 0) && (dsFlag != null && dsFlag.Tables.Count > 0))
                            {
                                if (ds.Tables[0].Rows.Count > 0 && ds.Tables[0].Rows[0]["Status"].ToString() == "1")
                                {
        ...
        protected void btnProceed_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (reg.UploadCorrectDocumentafterReporting(PID, Session["UserLoginID"].ToString(), UserInfo.GetIPAddress()))
                {
        ...
        private class dbUtility
        {
            ...
            public DataSet saveUploadedDocument(Int64 PID, Int64 DocumentID, string FileName, string AbsoluteFilePath, string OriginalFileName, string FileUploadedBy, string FileUploadedByIPAddress, char checkbarcode, string barcode)
            {
                SqlParameter[] param =
                    ...
                    new SqlParameter("@CandidateBarcode",SqlDbType.VarChar)
                };
            ...
            public DataSet UpdateDocumentIsSubmittedAtAFCFlagafterReporting(Int64 PID, Int64 DocumentID)
            {
                SqlParameter[] param =
                    {
                    new SqlParameter("@PID",SqlDbType.BigInt),
                ...
                try
                {
                    return db.ExecuteDataSet("MHDTE_spUpdateDocumentIsSubmittedAtAFCFlagafterReporting", param);
