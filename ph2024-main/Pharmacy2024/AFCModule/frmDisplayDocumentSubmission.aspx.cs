...
{
    public partial class frmDisplayDocumentSubmission : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                try
                {
                    ...
                    if (Global.DocumentVerification == 0 && Convert.ToInt32(Session["UserTypeID"].ToString()) != 21)
                    {
                        Response.Redirect("../ApplicationPages/WelcomePageAFC.aspx", true);
                    }
                    ...
                    for (Int32 i = 0; i < gvDocumentsNotSubmitted.Rows.Count; i++)
                    {
                        ...
                        {
                            EWSFlag += "As candidate has not submitted the " + DocumentDetails + "<br />";
                        }
                    }
            ...
            try
            {
                ...
                if (IsNCLValid == 1)
                {
                    if (BlockFlag.Length == 0)
                    {
                        ...
                        if (ProposedApplicationFeeToBePaid > 0)
                        {
                            if (reg.confirmApplicationFormProvisionally(PID, IsCandidatureTypeChanged, IsCategoryChanged, IsPHTypeChanged, IsDefenceTypeChanged, IsMinorityChanged, IsEWSChanged, IsOrphanChanged, txtComments.Text, IsNCLReceiptSubmitted, NCLIssueDate, 1, IsTFWSChanged, Session["UserLoginID"].ToString(), UserInfo.GetIPAddress()))
                            {
                                ...
                                if (Global.SendSMSToCandidate == 1)
                                {
                                    ...
                                    sMSTemplate.PID = PID;
                                    sMSTemplate.ConfirmedBy = ds.Tables[0].Rows[0]["ConfirmedBy"].ToString();
                                    //sMSTemplate.ConfirmedDateTime = ds.Tables[0].Rows[0]["ConfirmedDateTime"].ToString();
                ...
                {
                    shInfo.SetMessage("NCL Certificate is not valid till 31st March " + NextYear + ".", ShowMessageType.Information);
                }
            }
            catch (Exception ex)
            {
