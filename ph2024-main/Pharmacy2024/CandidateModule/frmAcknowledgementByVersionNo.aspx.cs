...
{
    public partial class frmAcknowledgementByVersionNo : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Convert.ToInt32(Session["UserTypeID"].ToString()) != 91 && Convert.ToInt32(Session["UserTypeID"].ToString()) != 23 && Convert.ToInt32(Session["UserTypeID"].ToString()) != 24 && Convert.ToInt32(Session["UserTypeID"].ToString()) != 21)
            {
            ...
            if (!IsPostBack)
            {
                try
                {
                    ...
                    {
                        PID = Convert.ToInt64(Session["UserLoginID"].ToString().Replace(Global.ApplicationFormPrefix, ""));
                    }
                    ...
                    if (!reg.isApplicationFormConfirmed(PID))
                    {
                        ...
                        shInfo.SetMessage("Application Form is not Confirmed. You can Print Acknowledgement after Confirmation Only.", ShowMessageType.Information);
                            ...
                            //{
                            //    ContentTable2.Visible = false;
                            //    shInfo.SetMessage("Your Scrutiny Mode Is Physical Verification you are not allowed to print Versionwise Acknowledgement.", ShowMessageType.Information);
                            //}
                    else
                    {
                        ddlApplicationFormVersion.DataSource = reg.GetApplicationFormVersionNo(PID, "Acknowledgement");
                        ...
                        ddlApplicationFormVersion.Items.Insert(0, new ListItem("-- Select Acknowledgement Version No --", "0"));
        ...
        protected void ddlApplicationFormVersion_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (ddlApplicationFormVersion.SelectedIndex != 0)
            {
                ...
                else
                {
                    ...
                    PersonalInformation obj = reg.GetPersonalInformationByAcknowledgementVersionNo(PID, Convert.ToInt32(ddlApplicationFormVersion.SelectedValue));
                    ...
                    if (obj.HSCDistrict == "NA")
                    {
                        ...
                        {
                            //Response.Redirect("../AFCModule/frmEditHomeDistrictAndCategoryDetails.aspx?PID=" + PID + "&Code=" + PID.ToString().GetHashCode(), true);
                        }
                    }
                    ...
                    imgSignature.ImageUrl = imgSignature1.ImageUrl= azObj.GetBlobContentAsBase64("Signature", UserInfo.GetImageURL(PID, "Signature"));
                    //imgPhotograph.ImageUrl = UserInfo.GetImageURL(PID, "Photograph");
                    //imgSignature.ImageUrl = imgSignature1.ImageUrl = UserInfo.GetImageURL(PID, "Signature");
                    ...
                    if (CandidatureTypeID == 8 || CandidatureTypeID == 9 || CandidatureTypeID == 10 || CandidatureTypeID == 11 || CandidatureTypeID == 12 || CandidatureTypeID == 13 || CandidatureTypeID == 14 || CandidatureTypeID == 18)
                    {
                    ...
                    List<DocumentDetails> lstDocuments = reg.GetPersonalInformationByAcknowledgementVersionNoDocuments(PID, Convert.ToInt32(ddlApplicationFormVersion.SelectedValue));
                    gvDocumentsSubmitted.DataSource = lstDocuments.Where(x => x.IsSubmittedAtFC == "Y").ToList(); //reg.getDocumentList(PID, "Y");
                    ...
                    gvDocumentsNotSubmitted.DataSource = lstDocuments.Where(x =>x.IsSubmittedAtFC == "N").ToList();// reg.getDocumentList(PID, "N");
                    ...
                    if (gvDocumentsNotSubmitted.Rows.Count > 0)
                    {
                        for (Int32 i = 0; i < gvDocumentsNotSubmitted.Rows.Count; i++)
                        {
                            ...
                            if (gvDocumentsNotSubmitted.Rows[i].Cells[1].Text == "Certificate of Indian Nationality in the Name of the Candidate.")
                            {
                                trNationalityDocumentNot.Visible = true;
                            }
                        }
                    ...
                    if (lstDocuments.Where(x => x.DocumentID.Contains("1") && x.IsSubmittedAtFC == "N").ToList().Count() > 0)
                    {
                        trNationalityDocumentNot.Visible = true;
                    }
                    //--SEBC Change Start--
                    //if (reg.GetPreviousCategoryDetailsOFSEBC(PID) && obj.FinalCategory=="Open" && obj.FinalAppliedForEWS == "Yes" && (lstDocuments.Where(x => x.DocumentID.Contains("36") && x.IsSubmittedAtFC == "N").ToList().Count() > 0) )
                    //{
                    //    trEWSNotSubmittedSEBC.Visible = true;
                    //}
                    //else
                    //    trEWSNotSubmittedSEBC.Visible = false;
                    //--SEBC Change End--

                    //--TFWS Change Start --
                    //if(obj.CourseApplied == "Pharmacy" && obj.FinalAppliedForTFWS == "Yes" && lstDocuments.Where(x => x.DocumentID.Contains("32") && x.IsSubmittedAtFC == "N").ToList().Count() > 0)
                    //trTFWSPH.Visible = true;
                    ////--TFWS Change End--
                    ///

                    List<DiscrepancyDetails> lstDiscrepancy = reg.GetAcknowledgementVersionNoDiscrepancy(PID, Convert.ToInt32(ddlApplicationFormVersion.SelectedValue));
                    if (lstDiscrepancy.Count > 0 && ScruType == "E")
                    {
                        tblDiscrepancy.Visible = true;
                        gvDiscrepancy.DataSource = lstDiscrepancy;
                        gvDiscrepancy.DataBind();
                    }
                    else
                    {
                        tblDiscrepancy.Visible = false;
                    }

                    DataSet EligibilityRemark = reg.getEligibilityFlag(PID);
                    ...
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        {
                            trComments.Visible = true;
                            lblComments.Text = obj.Comments.ToString();
                        }
