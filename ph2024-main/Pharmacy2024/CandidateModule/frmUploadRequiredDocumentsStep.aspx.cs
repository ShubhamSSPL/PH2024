...
namespace Pharmacy2024.CandidateModule
{
    public partial class frmUploadRequiredDocumentsStep : System.Web.UI.Page
    {
            ...
            if (flUpload.HasFile)
            {
                ...
                if (fileurl != "")
                {
                    ...
                    try
                    {
                        ...
                        int JuridictionID = 0;
                        int TalukaID = 0;
                        ds = new dbUtility().saveUploadedDocument(objSessionData.PID, DocID, sysfilename, uploadedFileURL, originalFileName, Convert.ToString(Session["UserLoginID"]), UserInfo.GetIPAddress(), checkBarcode, barcode, CandidateBarcode);
                        if (ds != null && ds.Tables.Count > 0)
                        {
        ...
        [AjaxMethod]
        public string UploadDoc(string base64String, string documentID, string hidJUD, string hidDID, string hidTID, string fileName, string fileExt, char checkBarcode, string barcode, string CandidateBarcode)
        {
            //ShowMessage shInfo = (ShowMessage)Master.FindControl("ShowMsg");
                ...
                if (extension != string.Empty)
                {
                    ...
                    if (fileurl != "")
                    {
                        ...
                        try
                        {
                            ...
                            int JuridictionID = 0;
                            int TalukaID = 0;
                            ds = new dbUtility().saveUploadedDocument(objSessionData.PID, Convert.ToInt16(documentID), sysfilename, uploadedFileURL, originalFileName, Convert.ToString(Session["UserLoginID"]), UserInfo.GetIPAddress(), checkBarcode, barcode, CandidateBarcode);
                            if (ds != null && ds.Tables.Count > 0)
                            {
                                if (ds.Tables[0].Rows.Count > 0 && ds.Tables[0].Rows[0]["Status"].ToString() == "1")
                                {
                                    if (Convert.ToInt16(documentID) == 21 || Convert.ToInt16(documentID) == 22)
                                    {
                                        ...
                                        dsJ = new dbUtility().saveUploadedDocumentJuridiction(objSessionData.PID, Convert.ToInt16(documentID), Convert.ToString(Session["UserLoginID"]), UserInfo.GetIPAddress(), JuridictionID, DistrictID, TalukaID);
                                        if (dsJ != null && dsJ.Tables.Count > 0)
                                        {
                                            if (dsJ.Tables[0].Rows.Count > 0 && dsJ.Tables[0].Rows[0]["Status"].ToString() == "1")
                                            {
        ...
        protected void btnProceed_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (BlockFlag.Length > 0)
                {
                    ...
                    LoadDocuments(objSessionDat.PID);
