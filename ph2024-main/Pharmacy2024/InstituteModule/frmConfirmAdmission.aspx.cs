...
{
    public partial class frmConfirmAdmission : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                try
                {
                    ...
                    DataSet dsReportingDetails = reg.getReportingDetailsByChoiceCode(PID, ChoiceCode, CAPRound, 'N');
        ...
        protected void btnProceed_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                string XMLStringV = "<DocumentList>";
                ...
                for (Int32 i = 0; i < gvDocuments.Rows.Count; i++)
                {
                    ...
                    {
                        XMLStringV += "<Document DocumentID=\"" + DocumentID + "\" IsSubmittedAtinstitute=\"" + "Y" + "\"></Document>";
                    }
                    else
                    {
                        XMLStringV += "<Document DocumentID=\"" + DocumentID + "\" IsSubmittedAtinstitute=\"" + "N" + "\"></Document>";
                ...
                XMLStringV += "</DocumentList>";
                if (Documentnotverified == 0)
                {
                    Int64 PID = Convert.ToInt64(Request.QueryString["PID"].ToString());
                    if (Global.IsOTPVerificationRequiredForAdmissionConfirm == 1)
                    {
                        tblOtp.Visible = true;
                        ...
                        if (synCommon.SendOTPSMS(sMSTemplate, SynCommon.TemplateSystemType.InstituteCAPOTP))
                        {
                    ...
                    else
                    {
                        Int64 ChoiceCode = Convert.ToInt64(Request.QueryString["ChoiceCode"].ToString());
                        Int32 CAPRound = Convert.ToInt32(Request.QueryString["CAPRound"].ToString());
                        DateTime AdmissionDate = Convert.ToDateTime(txtAdmissionDate.Text.Split("/".ToCharArray())[1] + "/" +
                                                                    txtAdmissionDate.Text.Split("/".ToCharArray())[0] + "/" +
                                                                    txtAdmissionDate.Text.Split("/".ToCharArray())[2]);
                        ...
                        reg.updateDocumentSubmissionForInstitute(PID, XMLString, Session["UserLoginID"].ToString(),
                            UserInfo.GetIPAddress());

                        if (AdmissionDate > DateTime.Now.Date)
                        {
                            shInfo.SetMessage("Admission Date should not be greater than Current Date.",
                                ShowMessageType.Information);
                        }
                        else
                        {
                            if (reg.confirmAdmissionByChoiceCode(PID, ChoiceCode, CAPRound, AdmissionDate, txtRemark.Text,
                            Session["UserLoginID"].ToString(), UserInfo.GetIPAddress()))
                            {
                                ...
                                if (Global.SendSMSToCandidate == 1)
                                    synCommon.SendInformationSMS(sMSTemplate, SynCommon.TemplateSystemType.ConfirmAdmissionCAP);
        ...
        protected void btnVerifyOtp_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (reg.verifyOTP(ApplicationID, txtOneTimePassword.Text, (int)SynCommon.TemplateSystemType.InstituteCAPOTP))
                {
                    Int64 ChoiceCode = Convert.ToInt64(Request.QueryString["ChoiceCode"].ToString());
                    ...
                    else
                    {
                        if (Documentnotverified == 0)
                        {
                            if (reg.confirmAdmissionByChoiceCode(PID, ChoiceCode, CAPRound, AdmissionDate, txtRemark.Text, Session["UserLoginID"].ToString(), UserInfo.GetIPAddress()))
                            {
                                SMSTemplate sMSTemplate = new SMSTemplate();
                                SynCommon synCommon = new SynCommon();
                                sMSTemplate.PID = PID;
