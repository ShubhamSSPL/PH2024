        ...
        protected void gvChoiceCodeList_RowCommand(object sender, System.Web.UI.WebControls.GridViewCommandEventArgs e)
        {
            ...
            try
            {
                ...
                if (obj.IsStateLevel == "Yes")
                    lblPWDCommon.Text = "PWD Common Reserved Seats : " + obj.PHOT.ToString();
                else
                    lblPWDCommon.Text = "PWD Common Reserved Seats : " + obj.PHHT.ToString();

                lblDEFCommon.Text = "DEF Common Reserved Seats : " + obj.DEFT.ToString();
            }
            catch (Exception ex)
            {
                Logging.LogException(ex, "[Page Level Error]");
