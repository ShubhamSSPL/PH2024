...
namespace Pharmacy2024.InstituteModule
{
    ...
    public partial class frmSeatMatrixVerificationStatusReport : System.Web.UI.Page
    {
        ...
        protected void GetData()
        {
            ...
            try
            {
                ...
                {
                    shInfo.SetMessage("You are not authorised to access this link.", ShowMessageType.Information);
                    ContentTable1.Visible = false;
                }

                gvInstituteList.DataSource = new GetSeatMatrixVerificationStatusReport().getSeatMatrixVerificcationStatusReport(Convert.ToInt32(ddlRegion.SelectedValue.ToString()), ddlVerificationStatus.SelectedValue.ToString(), ddlVerificationDetails.SelectedValue.ToString(), Global.CAPRound);
    ...
    public class GetSeatMatrixVerificationStatusReport
    {
        public DataSet getSeatMatrixVerificcationStatusReport(int RegionID, string VerificationStatus, string VerificationDetails, int CAPRound)
        {
            ...
            parameters[3].Value = CAPRound;
            DBConnection db = new DBConnection();
            try
            {
                return db.ExecuteDataSet("MHDTE_spGetSeatMatrixVerificcationStatus", parameters);
            }
            finally
            {
                db.Dispose();
