    ...
    public partial class frmSetPreferences : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                try
                {
                    ...
                    DataSet dsCurrentLink = reg.getCurrentLinkCAPRound2(objSessionData.PID, "frmSetPreferences.aspx");
                    ...
                    if (objSessionData.OptionFormStatusCAPRound2 == 'A' || objSessionData.StepIDCAPRound2 < 1)
                    {
                        Response.Redirect("../CandidateModuleCAPRound2/frmOptionForm.aspx?tms=101", true);
                    }

                    if (objSessionData.StepIDCAPRound2 >= 2)
                    {
                    ...
                    dgSetPreferences.DataSource = reg.getSelectedOptionsListCAPRound2(objSessionData.PID);
                    CheckCount = 0;
                    dgSetPreferences.DataBind();
                    if (dgSetPreferences.Items.Count == 0)
                    {
                        Response.Redirect("../CandidateModuleCAPRound2/frmShortListOptions.aspx?tms=101", true);
                    }
                }
                catch (Exception ex)
                {
        ...
        protected void btnProceed_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                switch (reg.savePreferencesCAPRound2(objSessionData.PID, PreferencesList, Session["UserLoginID"].ToString(), UserInfo.GetIPAddress()))
                {
                    case 0:
                        shInfo.Visible = true;
                    ...
                    case 1:
                        if (objSessionData.StepIDCAPRound2 < 2)
                        {
                            objSessionData.StepIDCAPRound2 = 2;
                        }
                        ...
                        break;
                    case 2:
                        Session["StepIDCAPRound2"] = 3;
                        Session["OptionFormStatusCAPRound2"] = "A";
